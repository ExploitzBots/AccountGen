...
async def shud(event):
    ...
    await hehe.edit("**Shudder Account**\n\nCombo: {email}:{password}\nEmail: {email}\nPassword: {password}\n\nGenerated By: @{event.sender.username}\nUser-ID: {event.sender_id}",
buttons=[
    [Button.url("Join Channel", Config.CHANNEL_URL)],
    [Button.inline("Back", data="gen")]
])
...
async def RetroCrush(event):
    ...
    with open('RetroCrush.txt') as ha:
        sedloif = ha.read().splitlines()
    sed = random.choice(sedloif)
    user_s = await randi.get_me()
    user_us = user_s.username
    ...
    await hehe.edit("**RetroCrush Account**\n\nCombo: {email}:{password}\nEmail: {email}\nPassword: {password}\n\nGenerated By: @{event.sender.username}\nUser-ID: {event.sender_id}",
buttons=[
    [Button.url("Join Channel", Config.CHANNEL_URL)],
    [Button.inline("Back", data="gen")]
])
...
async def MeetMe(event):
    ...
    await hehe.edit("**MeetMe Account**\n\nCombo: {email}:{password}\nEmail: {email}\nPassword: {password}\n\nGenerated By: @{event.sender.username}\nUser-ID: {event.sender_id}",
buttons=[
    [Button.url("Join Channel", Config.CHANNEL_URL)],
    [Button.inline("Back", data="gen")]
])
